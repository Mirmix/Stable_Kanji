...
def parse_args():
    ...
    parser.add_argument("--from_scratch", action="store_true", 
                       help="Train from scratch with random weights")
...
def reinit_weights(module):
    """Recursively reinitialize all weights in a module"""
    if hasattr(module, 'reset_parameters'):
        module.reset_parameters()
    for child in module.children():
        reinit_weights(child)
...
def main():
    ...
    if args.from_scratch:
        # Reinitialize the weights of UNet2DConditionModel if training from scratch
        logger.info("Reinitializing UNet weights...")
        reinit_weights(unet)
